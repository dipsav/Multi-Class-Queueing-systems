...
def simulation(  requestArrivalRate = 5.0,
    ...
    for sr in xrange(numberOfServer):
        ...
        for sk in xrange(len(skillDistribution)):
            serverSkillScore[sr][sk] = 1.0/skillServerRates[sr][sk] * skillServerCosts[sr][sk] if (skillServerRates[sr][sk]) else float('inf')
...
def run_time_test_single_item():
    ...
    serviceRates = np.array([0.5, 2.0, 1.0/2, 2.0])
